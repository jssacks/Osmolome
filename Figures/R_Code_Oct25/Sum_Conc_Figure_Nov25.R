...
p1.env <- ggplot(p1.env.dat) +
  ...
  scale_y_continuous(name = expression(POC~"(" * mu * "M)"), sec.axis = sec_axis(trans=~.*(1/15), name=NULL), limits = c(0,4.5),
                     expand = c(0,NA)) +
...
p3.env <- ggplot(p3.env.dat, aes(x = Station, y = Value)) +
  ...
  scale_y_continuous(name = NULL, sec.axis = sec_axis(trans=~.*(1/50), name=Chla~"(mg"~m^{-3}*")"), 
...
poc.cor.fig <- ggplot(correlation.fig.dat, aes(x = poc, y = Sum.Part.Conc.nM)) +
  ...
  theme(legend.position = "none") +
  ylab("Particulate Conc. (nM)") +
  xlab(expression(POC~"(" * mu * "M)")) + 
  annotate("text", x = 1.3, y = 300, 
           label = expression(atop(R^2 == 0.82, italic(p) < 0.001)),
           hjust = 0)
poc.cor.fig
...
chla.cor.fig <- ggplot(correlation.fig.dat %>%
  ...
  ylab("Part. Conc. (nM)") +
  xlab(expression(Chla~"(mg"~m^{-3}*")")) + 
  annotate("text", x = 0.05, y = 300, 
           label = expression(atop(R^2 == 0.78, italic(p) < 0.001)),
           hjust = 0)
chla.cor.fig
...
atp.cor.fig <- ggplot(atp.osmo.dat, aes(x = Mean.living.biomass.umolC.L, y = Mean.Osmo.Conc.nM)) +
 ...
 # scale_fill_manual(values = c("darkgray", "cornflowerblue")) +
  ...
  ylab("Part. Conc. (nM)") +
  xlab(expression(Living~Biomass~"(" * mu * "M C)")) +
  theme(legend.position = "none",
        axis.title.y = element_blank()) + 
  annotate("text", x = 0.5, y = 35, 
           label = expression(atop(R^2 == 0.72, italic(p) < 0.001)),
           hjust = 0)
...
biovol.cor.fig <- ggplot(correlation.fig.dat %>%
  ...
  scale_y_log10() +
  ylab("Part. Conc. (nM)") +
  xlab(expression(Biovolume~"("*mu*L~L^{-1}*")")) + 
  annotate("text", x = 0.05, y = 32, 
           label = expression(atop(R^2 == 0.85, italic(p) < 0.001)),
           hjust = 0)
biovol.cor.fig
...
ggsave(distribution.plots, filename = "Figures/Output_Oct25/Distribution_Plot.png",
       dpi = 300, width = 7.5, height = 6.25, scale = 1.5)
