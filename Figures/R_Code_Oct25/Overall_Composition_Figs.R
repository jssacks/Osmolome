...
p4.dat.d <- dat.panel %>%
  ...
  reframe(Mean.Part.nM = mean(Part.Conc.nM, na.rm = TRUE),
          Mean.Diss.nM = mean(Diss.Conc.nM, na.rm = TRUE)) %>%
  full_join(., p4.dat.p %>% select(Station) %>% unique())
...
comb.plot.1 <- (part.class.region.plot + plot_spacer() + diss.class.region.plot + plot_layout(guides = "collect", widths = c(3, 0.05, 2))) / 
  plot_spacer() /
  ((p.p1 + p.p2 + p.p3 + p.p4 + plot_layout(nrow = 1, widths = c(3.2, 3.2, 2.25, 1.25))) / 
                  (d.p1 + d.p2 + d.p3 + d.p4 + plot_layout(nrow = 1, widths = c(3.2, 3.2, 2.25, 1.25))) + plot_layout(guides = "collect")) +
  plot_layout(heights = c(2, 0.5, 4.5)) 
comb.plot.1

ggsave(comb.plot.1, filename = "Figures/Output_Oct25/Relative_Composition_Figure_Nov25.png",
       dpi = 1000, height = 6.5, width = 8, scale = 1.4)
