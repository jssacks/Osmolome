...
#Define inputs
raw.file <- "Intermediates/G2_osmo_data_raw.csv"
...
qc.dat.export <- qc.dat %>%
  mutate(detected = case_when(is.na(min.area.flag) & is.na(blk.ratio.flag) ~ "Yes",
                              TRUE ~ "No")) %>%
  select(Rep, Compound, detected)


write_csv(qc.dat.export, file = "Intermediates/G2SF_QCdat.csv")
...
# write_csv(dat.qc.remove, file = "Intermediates/G2SF_osmo_QCdat_samplesremoved.csv")
# write_csv(dat.qc.impute, file = "Intermediates/G2SF_osmo_QCdat_blanksimputed.csv")
# 
# 
