...
###Calculate Blk thresholds, Remove blank from km1906 that is an order of magnitude higher than the other blank as it shows clear evidence of contamination
blk.dat <- raw.dat %>%
...
###Evaluate compounds in each sample based on QC performance
qc.dat.export <- qc.dat %>%
  mutate(detected = case_when(is.na(min.area.flag) & is.na(blk.ratio.flag) ~ "Yes",
                              TRUE ~ "No")) %>%
  select(Cruise, Rep, Compound, detected)


###
write_csv(qc.dat.export, file = "Intermediates/Particulate_QCdat.csv")
