...
###organize and summarize qc:
qc.dat.export <- qc.dat %>% 
  mutate(detected = case_when(is.na(min.area.flag) & is.na(blk.lod.flag) ~ "Yes",
                            TRUE ~ "No")) %>%
  select(Cruise, Rep, Compound, detected)


###
write_csv(qc.dat.export, file = "Intermediates/Dissolved_QCdat.csv")
